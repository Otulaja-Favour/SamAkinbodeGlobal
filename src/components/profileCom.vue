...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      this.transactionHistory = JSON.parse(
        localStorage.getItem(`transactionHistory_${userId}`) || '[]',
      )
  ...
  methods: {
    ...
    async addCommentToBook() {
      ...
      try {
        const comments = this.currentBook.comments
          ? [...this.currentBook.comments, this.commentText]
          : [this.commentText]
